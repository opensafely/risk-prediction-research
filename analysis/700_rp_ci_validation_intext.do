********************************************************************************
*
*	Do-file:			700_rp_ci_validation_intext.do
*
*	Written by:			Fizz
*
*	Data used:			data/
...
*							susp_coefs.dta
*
*	Data created:		data/model_ci_`tvc'_`r'.dta
*							where tvc = foi, ae or susp.
*
*	Other output:		Log file:  	700_rp_ci_validation_intext_`tvc'.log
*						Estimates:	coefs_ci_`tvc'_`r'.ster
*
********************************************************************************
*
...
* Open a log file
capture log close
log using "./output/700_rp_ci_validation_intext_`tvc'", text replace
...
forvalues r = 1 (1) 8 {
	...
	*********************
	...
	capture erase output/models/coefs_ci_`tvc'_`r'.ster
	...
	* Fit model
	...
	estimates save output/models/coefs_ci_`tvc'_`r', replace
	...
	* Add baseline survival to matrix (and add a matrix column name)
	...
	get_coefs, coef_matrix(b) eqname("_t:") cons_no ///
		dataname("data/model_ci_`tvc'_`r'")
