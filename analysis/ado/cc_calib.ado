...
program define cc_calib, rclass
	...
	* Weight condition
	if "`weight'"!="" {
		local weightcond = "[pweight=`weight']" 
		local iweightcond = "[pweight=`weight']" 
	} 
	else {
		local weightcond = ""
		local iweightcond = ""
	...
	* Obtain groups by deciles (in orig cohort, i.e. weighted)
	tempvar gp_coh 
	qui xtile `gp_coh' = `predicted' `weightcond', nq(`nq')
	...
	*  Obtain estimates of observed and expected in each group in orig cohort (weighted)
	...
	forvalues i = 1 (1) 10 {
		qui summ `predicted' if `gp_coh'==`i' `iweightcond'
		qui replace `exp_events' = r(sum)  if `gp_coh'==`i'
		qui replace `pbar'       = r(mean) if `gp_coh'==`i'
		
		qui summ `observed' if `gp_coh'==`i' `iweightcond'
		qui replace `obs_events' = r(sum) if `gp_coh'==`i'
		
		qui summ `cons' if `gp_coh'==`i' `iweightcond'
	...
	* Intercept (include logit of predicted risk as intercept)
	...
	qui logit `observed' `weightcond', offset(`lp')
	...
	* Slope 
	qui logit `observed' `lp' `weightcond'
	...
	*********************
	...
	noi di "Mean observed risk:	" 		_col(50) `mean_pred'
