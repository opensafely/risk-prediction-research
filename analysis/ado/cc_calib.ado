...
program define cc_calib, rclass
	...
	syntax varlist(min=2 max=2), weight(varname) 	///
		[pctile nq(integer 10)] data(string)
	...
	* Obtain groups by deciles (in orig cohort, i.e. weighted)
	tempvar gp_coh 
	qui xtile `gp_coh' = `predicted' [pweight=`weight'], nq(`nq')
	...
	*  Obtain estimates of observed and expected in each group in orig cohort (weighted)
	...
	forvalues i = 1 (1) 10 {
		qui summ `predicted' if `gp_coh'==`i' [iweight=`weight']
		qui replace `exp_events' = r(sum)  if `gp_coh'==`i'
		qui replace `pbar'       = r(mean) if `gp_coh'==`i'
		
		qui summ `observed' if `gp_coh'==`i' [iweight=`weight']
		qui replace `obs_events' = r(sum) if `gp_coh'==`i'
		
		qui summ `cons' if `gp_coh'==`i' [iweight=`weight']
	...
	* Intercept (include logit of predicted risk as intercept)
	...
	qui logit `observed' [pweight=`weight'], offset(`lp')
	...
	* Slope 
	qui logit `observed' `lp' [pweight=`weight']
