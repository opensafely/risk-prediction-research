...
tabulatevariable, variable(agegroup) start(1) end(6) outcome(`outcome') 
file write tablecontent _n 
...
file write tablecontent _n
*ASTHMA
tabulatevariable, variable(asthmacat) start(2) end(3) outcome(`outcome') /*no ocs, then with ocs*/
*CARDIAC
tabulatevariable, variable(chronic_cardiac_disease) start(1) end(1) outcome(`outcome')
file write tablecontent _n 
*DIABETES
tabulatevariable, variable(diabcat) start(2) end(4) outcome(`outcome') /*controlled, then uncontrolled, then missing a1c*/
file write tablecontent _n
*CANCER EX HAEM
...
*DIALYSIS
tabulatevariable, variable(dialysis) start(1) end(1) outcome(`outcome')
*LIVER
...
*ORGAN TRANSPLANT
tabulatevariable, variable(organ_transplant) start(1) end(1) outcome(`outcome')
*SPLEEN
tabulatevariable, variable(spleen) start(1) end(1) outcome(`outcome')
*RA_SLE_PSORIASIS
tabulatevariable, variable(ra_sle_psoriasis) start(1) end(1) outcome(`outcome')
*OTHER IMMUNOSUPPRESSION
tabulatevariable, variable(other_immunosuppression) start(1) end(1) outcome(`outcome')
